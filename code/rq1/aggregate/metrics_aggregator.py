...
def initialize_samples(base_data: dict, dataset: str) -> list:
    """Initialize the samples structure with all data except metrics."""
    samples = []
    for sample in base_data[dataset]["samples"]:
        aggregated_sample = {
            "question": sample.get("question", ""),
            "complexity_level": sample.get("complexity_level", 0),
            "model": sample.get("model", ""),
            "response": sample.get("response", ""),
            "metrics": {}  # metrics will be merged later
        }
...
def apply_metric_value(target_metrics: dict, metric_type: str, subcategories: list, value: dict) -> None:
    ...
    if not subcategories:
        # Directly assign if there are no subcategories
        target_metrics[metric_type] = value
        return

    # Traverse and create nested dictionaries for subcategories
    ...
    # Assign value at the deepest subcategory level
...
def find_metric_sample(samples: list) -> dict:
    """Find the first sample with metrics."""
...
def determine_metric_info(file_data: dict, dataset: str) -> tuple[str, list]:
    ...
    metric_sample = find_metric_sample(samples)
    if not metric_sample:
        return None, None
        
    metrics = metric_sample["metrics"]
    if not metrics:
        return None, None
        
    metric_type = list(metrics.keys())[0]
    subcategories = get_metric_subcategories(metric_type, config)
    return metric_type, subcategories


def calculate_metric_average_for_dict(models: dict, metric_name: str, metric_value: dict) -> dict:
    """Calculate average for dictionary-type metrics."""
    nested_avg = {}
    for sub_key in metric_value:
        sub_values = [
            models[model][metric_name][sub_key] 
            for model in models.keys()
            if model in models and 
               metric_name in models[model] and 
               sub_key in models[model][metric_name]
        ]
        if sub_values:
...
def calculate_metric_average_for_number(models: dict, metric_name: str) -> float:
    """Calculate average for numeric metrics."""
    values = [
        models[model][metric_name] 
        for model in models.keys()
        if model in models and metric_name in models[model]
    ]
...
def calculate_llm_average(aggregated_samples: list) -> None:
    ...
    for sample in aggregated_samples:
        ...
        if "llm" not in metrics or not isinstance(metrics["llm"], dict) or len(metrics["llm"]) <= 1:
            continue
            
        models = metrics["llm"]
        model_names = [name for name in models.keys() if not should_exclude_model(name)]
        ...
        for metric_name, metric_value in first_model.items():
            if isinstance(metric_value, str):
                continue  # Skip string values (e.g. labels or text)
                
            if isinstance(metric_value, dict):
                # Nested metric values
                nested_avg = calculate_metric_average_for_dict(models, metric_name, metric_value)
                average_metrics[metric_name] = nested_avg
            elif isinstance(metric_value, (int, float)):
                # Flat numerical value
                avg_value = calculate_metric_average_for_number(models, metric_name)
                average_metrics[metric_name] = avg_value
        
        # Store the average as a special "average" model entry
...
def merge_configs(configs: list) -> dict:
    """Merge multiple configs, prioritizing non-default values."""
    if not configs:
        return {}
        ...
        for key, value in config.items():
            should_update = (
                ...
                (isinstance(value, bool) and 
                 key.startswith("disable_") and 
                 merged[key] is True and 
                 value is False)
            )
            if should_update:
                merged[key] = value
...
def aggregate_data_for_dataset(dataset: str, file_list: list[Path]) -> dict:
    ...
    for file in file_list:
        ...
        if "config" in file_data[dataset]:
            configs.append(file_data[dataset]["config"])
